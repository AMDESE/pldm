...
class GetPDR : public CommandInterface
{
  ...
  private:
                                 ...
                                 ordered_json& output)
    {
        ...
        int rc =
            decode_redfish_resource_pdr_data(data, (size_t)dataLength, &pdr);
        if (rc != PLDM_SUCCESS)
        {
        ...
        output["ProposedContainingResourceName"] = getRdeResourceName(
            pdr.prop_cont_resrc_name, pdr.prop_cont_resrc_length);
        output["SubURILengthBytes"] = pdr.sub_uri_length;
        output["SubURI"] =
                 ...
                 }))
        {
            ...
            output.emplace("AdditionalResourceSubURILengthBytes[" +
                               std::to_string(i) + "]",
                           res->length);
            output.emplace("AdditionalResourceSubURI[" + std::to_string(i) +
                               "]",
                           getRdeResourceName(res->name, res->length));
        }
        ...
        output["MajorSchemaName"] =
            getRdeResourceName(pdr.major_schema.name, pdr.major_schema.length);
        output["OEMCount"] = pdr.oem_count;

        for (auto [i, oem] :
             std::views::iota(0u, static_cast<unsigned>(pdr.oem_count)) |
                 std::views::transform([&](size_t i) {
                     return std::pair{i, pdr.oem_list[i]};
                 }))
        {
            output.emplace("OEMNameLengthBytes[" + std::to_string(i) + "]",
                           oem->length);
            output.emplace("OEMName[" + std::to_string(i) + "]",
                           getRdeResourceName(oem->name, oem->length));
        }

        free_redfish_resource_pdr_data(&pdr);
    }

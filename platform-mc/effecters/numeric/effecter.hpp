...
#include <xyz/openbmc_project/Control/NumericEffecter/server.hpp>
...
using NumericEffecterInherit = sdbusplus::server::object_t<
    sdbusplus::xyz::openbmc_project::Control::server::NumericEffecter>;
using DataSize = sdbusplus::common::xyz::openbmc_project::control::NumericEffecter::DataSize;
using OperationalStates = sdbusplus::common::xyz::openbmc_project::control::NumericEffecter::OperationalStates;

class NumericEffecterDbusIntf : public NumericEffecterInherit
{
  public:
    NumericEffecterDbusIntf(sdbusplus::bus_t& bus, const char* path, pldm::platform_mc::NumericEffecter& nE);
    virtual ~NumericEffecterDbusIntf() = default;
    void handleValueChange(pldm::platform_mc::NumericEffecter& nE,
                           pldm_effecter_oper_state effecterOperState,
                           double pendingValue, double presentValue, uint8_t sizeEnum);
    void handleError(pldm::platform_mc::NumericEffecter& nE);

    /** Get value of EffecterValue */
    std::vector<uint8_t> effecterValue() const override;
    /** Set value of EffecterValue */
    std::vector<uint8_t> effecterValue(std::vector<uint8_t> value) override;

    /** Get value of PresentValue */
    std::vector<uint8_t> presentValue() const override;
    /** Set value of PresentValue (Local use only) */
    std::vector<uint8_t> presentValue(std::vector<uint8_t> value) override;

    /** Get value of PendingValue */
    std::vector<uint8_t> pendingValue() const override;
    /** Set value of PendingValue (Local use only) */
    std::vector<uint8_t> pendingValue(std::vector<uint8_t> value) override;

    /** Get value of EffecterDataSize */
    DataSize effecterDataSize() const override;
    /** Set value of EffecterDataSize (Local value only) */
    DataSize effecterDataSize(DataSize value) override;

    /** Get value of EffecterOperationalState */
    OperationalStates effecterOperationalState() const override;

    /** Set value of EffecterOperationalState */
    OperationalStates effecterOperationalState(OperationalStates value) override;

  private:
    pldm::platform_mc::NumericEffecter& nEffecter;
};
...
class NumericEffecter
{
  public:
    NumericEffecter(const pldm_tid_t tid,
                    ...
                    const std::string& associationPath, Terminus& terminus,
                    TerminusManager& terminusManager, exec::async_scope& scope);
    ~NumericEffecter() {};
    ...
    void setNumericEffecterValueAsync(double v);

    void getNumericEffecterValueAsync();
     ...
     */
    void registerInterface(std::unique_ptr<pldm::platform_mc::NumericEffecterDbusIntf> intf);
  ...
  private:
    ...
    std::unique_ptr<pldm::platform_mc::NumericEffecterDbusIntf> numericEffecterDbusIntf = nullptr;
    ...
    std::vector<std::unique_ptr<pldm::platform_mc::NumericEffecterDbusIntf>> interfaces;

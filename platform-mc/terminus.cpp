    ...
    for (auto& pdr : pdrs)
    {
        ...
        switch (pdrHdr->type)
        {
            ...
            case PLDM_EFFECTER_AUXILIARY_NAMES_PDR:
            {
                ...
                lg2::info(
                    "Parsed Effecter Auxiliary Names PDR with type {TYPE} handle {HANDLE}, effecter id {ID}",
                    "TYPE", pdrHdr->type, "HANDLE",
                    static_cast<uint32_t>(pdrHdr->record_handle),
                    "ID", std::get<0>(*effecterAuxNames));
            ...
            case PLDM_NUMERIC_EFFECTER_PDR:
            {
                ...
                lg2::info(
                    "Parsed Numeric Effecter PDR with type {TYPE} handle {HANDLE} effecter id {ID}",
                    "TYPE", pdrHdr->type, "HANDLE",
                    static_cast<uint32_t>(pdrHdr->record_handle),
                    "ID", static_cast<uint32_t>(parsedPdr->effecter_id));
...
std::shared_ptr<pldm::platform_mc::EffecterAuxiliaryNames> Terminus::getEffecterAuxiliaryNames(
    pldm::platform_mc::EffecterID id)
{
    auto it = std::find_if(
        effecterAuxiliaryNamesTbl.begin(), effecterAuxiliaryNamesTbl.end(),
        [id](const std::shared_ptr<pldm::platform_mc::EffecterAuxiliaryNames>&
...
std::shared_ptr<pldm::platform_mc::EffecterAuxiliaryNames>
...
#pragma GCC diagnostic pop
    ...
    return std::make_shared<pldm::platform_mc::EffecterAuxiliaryNames>(
    ...
    Terminus::parseNumericEffecterPDR(const std::vector<uint8_t>& pdr)
{
    ...
    auto rc =
        decode_numeric_effecter_pdr_data_local(ptr, pdr.size(), parsedPdr.get());
    if (rc)
    {
        return nullptr;
    }
...
std::vector<std::string> Terminus::getEffecterNames(
    const pldm::platform_mc::EffecterID& effecterId)
{
         ...
         std::views::iota(0, static_cast<int>(effecterCount)))
    {
        ...
        for (const auto& [languageTag, name] : nameMap[i])
        {
            if (languageTag.find("en") != std::string_view::npos && !name.empty())
            {

...
struct ResourceInfo
{
    ...
    std::string uri;           ///< Redfish URI (e.g., /redfish/v1/Chassis/1)
    uint16_t schemaClass;      ///< Schema class identifier (PLDM)
...
class ResourceRegistry
{
  public:
     ...
     * @param eid Entity ID identifying the device.
     * @param parent Pointer to parent object for hierarchical access.
     */
    explicit ResourceRegistry(uint16_t eid, void* parent = nullptr);

    /**
     * @brief Register a resource and its schema metadata.
     * @param resourceId The resource ID (e.g., "Chassis_1")
     * @param info The metadata associated with the resource.
     */
     ...
     * @param path File path to save the registry.
     */
  ...
  private:
    ...
    std::unordered_map<std::string, ResourceInfo>
        resourceMap_;     // Map from URI to metadata

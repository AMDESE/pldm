...
class MultipartSender
{
  public:
     ...
     */
    void setTransferFlag(uint8_t tf)
    {
        transferFlag_ = tf;
    }

    std::vector<uint8_t> getFromTransferBytes(uint32_t requestPayloadLength);

    std::vector<uint8_t> getChunks(uint32_t requestPayloadLength,
                                   bool isFinalChunk);
  ...
  private:
    ...
    std::span<const uint8_t> dataPayload_;
    uint32_t dataPayloadLength_;
    bool complete = false;
};
